...
- [TextEmbeddingGenerationService Interface](https://github.com/microsoft/semantic-kernel/tree/main/java)

**Prompt 1: Add Embedding Service to Kernel**
```
Update the kernel builder to include text embedding generation:
- Add the text-embedding-3-small model using OpenAITextEmbeddingGenerationService
- Use the same OpenAI async client configured for GitHub Models
- Get the embedding service from the kernel after building it using kernel.getService(TextEmbeddingGenerationService.class)
```
...
```
Create a loop to:
- Generate an embedding for each sentence using embeddingService.generateEmbeddingsAsync
- Store each embedding vector in a List<List<Float>>
...
> **Note:** The `cosineSimilarity` method in App.java was written by Copilot with a single prompt: "Write a method to calculate cosine similarity." If you'd like to practice, feel free to delete it and try generating it yourself with that prompt!
...
Choose 2-3 experiments above and modify the `sentences` list in your App.java. Run the program each time and observe how the similarity scores change. Write down:
