...
```
Create a Java class called SentenceRecord with the following properties:
- id of type String with @VectorStoreRecordKey annotation
- text of type String with @VectorStoreRecordData annotation
- embedding of type List<Float> with @VectorStoreRecordVector annotation and dimension of 1536
- createdAt of type OffsetDateTime with @VectorStoreRecordData annotation

Include the necessary import statements for com.microsoft.semantickernel.data annotations.
Include a constructor that takes all fields as parameters.
Include getter methods for all fields.
```

**What to Look For:** GitHub Copilot should create a Java class that defines the structure of data you'll store in the vector database. The annotations tell Semantic Kernel which property is the key, which is the vector, and which is regular data.
...
- [Java Semantic Kernel Vector Stores](https://github.com/microsoft/semantic-kernel/tree/main/java) - You may need to add Maven dependencies
...
2. Loop through each sentence in the sentences list
3. For each sentence:
...
```
Create a new static method called searchSentencesAsync that:
...
**Test Point**: Run the application using `mvn clean compile exec:java` or the "maven build" task. You should see:
