...
**Prompt 1: Import MemoryVectorStore**
```
Import MemoryVectorStore from @langchain/community/vectorstores/memory at the top of app.js.
```

**What to Look For:** The MemoryVectorStore should be imported. In JavaScript with LangChain, you don't need to define a separate data model class - the vector store handles the structure automatically. Each document will have:
...
- [LangChain.js Memory VectorStore](https://js.langchain.com/docs/integrations/vectorstores/memory)

**Prompt 2: Create MemoryVectorStore**
```
After initializing the embeddings model in main(), create a MemoryVectorStore instance:
- Create it with MemoryVectorStore.fromTexts() method
- Pass an empty array [] as the initial texts parameter
- Pass the embeddings instance as the second parameter
- Store it in a variable called vectorStore
```
...
1. Create the sentences array with 3 test sentences (same as Lab 1)
2. Use the vectorStore.addDocuments() method to add all sentences at once
...
**📚 Reference:** [LangChain.js Vector Store Retrieval](https://js.langchain.com/docs/how_to/vectorstores/) - Learn about vector similarity search methods.

**Prompt 4: Create a Semantic Search Function**
```
Create a function called searchSentences that:
- Takes parameters: the vectorStore and a search query string
- Optionally takes k (default to 3) for the number of results to return
- Uses vectorStore.similaritySearchWithScore() to find similar documents
- Returns the top k most similar results with their similarity scores
- Print the results with formatting showing:
  - Rank number (1, 2, 3...)
  - Similarity score formatted to 4 decimal places using .toFixed(4)
  - The sentence text
```
...
1. Import readline from 'readline' at the top of the file
2. Display a header "=== Semantic Search ==="
3. Create a readline interface using readline.createInterface()
4. Use the interface to prompt the user "Enter a search query (or 'quit' to exit): "
5. If the user types "quit" or "exit", close the interface and exit
6. If the input is empty or whitespace only, prompt again
7. Otherwise, call the searchSentences function with the user's query
8. Display the results
9. Print a blank line between searches for readability
10. After the loop ends, print a goodbye message

Note: Use async/await with the readline interface for better control flow.
```

### Step 6: Test Your Semantic Search

**Test Point**: Run the application using `node app.js`. You should see:
...
```
Replace MemoryVectorStore with a persistent vector store like Chroma or FAISS.
Add the ability to save and reload the vector store from disk.
Ask Copilot: "Replace MemoryVectorStore with a persistent vector store like Chroma to save data to disk"
```
