...
**What to Look For:** Your Program.cs should still initialize the kernel, embedding service, and vector store collection, but no longer populate it with sentences.
...
**📚 Reference:** [File I/O in .NET](https://learn.microsoft.com/en-us/dotnet/standard/io/) - Learn about reading files asynchronously.

**Prompt 3: Create File Loading Function**
```
Create an async method called LoadDocumentAsync that:
- Takes parameters: the embedding service, the vector store collection, and a file path
- Reads all text from the file using File.ReadAllTextAsync
- Generates an embedding for the entire document content
- Creates a new DocumentRecord with:
  - Id: A GUID
  - FileName: The file name from the path (use Path.GetFileName)
...
**Test Point**: Run the application using `dotnet run`. 
...
**Test Point**: Run the application again using `dotnet run`.
...
**Test Again**: Run `dotnet run` one more time.
